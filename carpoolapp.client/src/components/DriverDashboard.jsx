            ...
            {ridesWithRequests.length === 0 ? (
                <p><i>No rides offered yet.</i></p>
                    ...
                    ridesWithRequests.map((ride) => {
                        const hasPassengers = ride.acceptedPassengers?.length > 0 || ride.requests?.length > 0;
                        ...
                        return (
                            <div key={ride.rideId} style={{ border: "1px solid #ccc", padding: "15px", marginBottom: "20px" }}>
                                ...
                                {/* Show Message button only if there are accepted passengers or pending requests */}
                                {hasPassengers && (
                                    <button
                                        onClick={() => navigate(`/chat/${ride.rideId}`)}
                                        style={{ background: "#007bff", color: "white", padding: "8px 12px", border: "none", cursor: "pointer", marginBottom: "10px" }}
                                    >
                                        Message
                                    </button>
                                )}
